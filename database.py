...
class NEODatabase:
    ...
    def __init__(self, neos, approaches):
        ...
        self.neos_by_des = {}
        self.neos_by_name = {}
        for index, item in enumerate(self._neos):
            self.neos_by_des[item.designation.lower()] = index
            if item.name is not None:
                self.neos_by_name[item.name.lower()] = index

        for approach in self._approaches:
            des_lower = approach._designation.lower()
            linked_neo_index = self.neos_by_des.get(des_lower, -1)
            if (linked_neo_index != -1):
                linked_neo = self._neos[linked_neo_index]
                linked_neo.approaches.append(approach)
                approach.neo = linked_neo
    ...
    def get_neo_by_designation(self, designation):
        ...
        or `None`.
        """
        neo_index = self.neos_by_des.get(designation.lower(), -1)
        if (neo_index != -1):
            return self._neos[neo_index]
        return None
    ...
    def get_neo_by_name(self, name):
        ...
        """
        neo_index = self.neos_by_name.get(name.lower(), -1)
        if (neo_index != -1):
            return self._neos[neo_index]
        return None

    def query(self, filters=()):
        """Query close approaches matching filters.
