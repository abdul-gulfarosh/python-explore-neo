        ...
        if (len(results) > 0):
            rows = [
                [x.time_str,
                    ...
                    x.neo.name,
                    x.neo.diameter,
                    x.neo.hazardous] for x in results]
...
def write_to_json(results, filename):
    ...
    with open(filename, 'w') as out_file:
        rows = [{
            'datetime_utc': x.time_str,
            'distance_au': x.distance,
            'velocity_km_s': x.velocity,
            'neo': {
                'designation':  x.neo.designation,
                'name': x.neo.name,
                'diameter_km': x.neo.diameter,
                'potentially_hazardous': x.neo.hazardous
                }
            } for x in results]
        json.dump(rows, out_file)
