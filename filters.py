...
class AttributeFilter:
    ...
    def __init__(self, op, value):
        """Construct a new `AttributeFilter` from a predicate and a value.
...
class DistanceFilter(AttributeFilter):
    """A subclass for filters on distance attribute."""
...
class DateFilter(AttributeFilter):
    """A subclass for filters on date attribute."""
...
class VelocityFilter(AttributeFilter):
    """A subclass for filters on velocity attribute."""
...
class DiameterFilter(AttributeFilter):
    """A subclass for filters on diameter attribute."""
...
class HazardousFilter(AttributeFilter):
    """A subclass for filters on hazardous attribute."""
...
def create_filters(
    ...
    `NEODatabase`
    because the main module directly passes this result to that method. For
    now, this can be thought of as a collection of `AttributeFilter`s.
...
def limit(iterator, n=None):
    ...
    """
    no_limit = n == 0 or n is None
    current_item_no = 0
    for item in iterator:
        if (no_limit or current_item_no != n):
            current_item_no += 1
            yield item
