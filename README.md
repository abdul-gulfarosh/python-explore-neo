...
Using the tool, you'll be able to inspect the properties of the near-Earth objects in the data set and query the data set of close approaches to Earth using any combination of the following filters:
...
## Dependencies
Python 3.6 is required to run the command line tool. Check usage section for more details

## Near-Earth Object Close Approach Datasets
...
NASA's web service lets you download their data on near-Earth objects in a CSV format. For this project, the data set used (`neos.csv`) comes directly from a query in which we limited the "Object Group" to NEOs and in which we selected _every_ output field. That's a _lot_ of columns (75, to be exact)!
...
What do each of the fields mean? [Check NASA's API documentation for more info](https://ssd-api.jpl.nasa.gov/doc/cad.html)
